...
## Использование пока что только через сборку (потому что у автора изменений не было желания собирать под разные ОС исполняемые файлы)
...
* проверить наличие домена в src_list.json(лежит в builds/ваша_ОС) и добавить
* если вдруг файла нет рядом с grabber_***_x64, то положить из ReadmangaGrabber/config
* если это не помогает, попробовать в корне проекта lib_urls.json обновить (добавить домены), в ReadmangaGrabber/config src_list.json обновить
* удалить файлы, что уже были в папке builds/ваша_ОС и собрать заново
...
## Сохранение токена для Mangalib
1. Войдите со своими логином/паролем на нужный сайт
2. Обновите страницу на всякий случай
3. (**НЕ ПЕРЕДАВАЙТЕ И НЕ ПУБЛИКУЙТЕ СВОЙ ТОКЕН ДОСТУПА**)
Находясь на сайте получите токен. Например в Chrome: F12 → Application/Storage → Local Storage → mangalib.me →  нажать на ключ "auth" → найти в новом окошке "token" со стрелочкой для выпадающего списка → скопировать длинный набор символов из "access_token" правой кнопкой мыши.
4. В настройках утилиты в самом внизу добавьте токен доступа. 

Токен действует месяц. Его придется обновлять регулярно.
